...
module "vmss" {
  ...
  # vmss_win_se_settings_script   = each.value.vmss_win_se_settings_script
  # vmss_win_se_settings_data     = each.value.vmss_win_se_settings_data
  vmss_win_se_settings = each.value.vmss_win_se_settings
  # vmss_custom_data     = local.vmss_custom_data_data
  vmss_custom_data = (
    each.key == "linux" ?
    base64encode(templatefile("${path.module}/cloud-init.tpl", {})) :
    null
